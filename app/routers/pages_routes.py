...
function setMainEtaFromOrders(orders){
  ...
  const bar = document.getElementById('etaFill') || document.getElementById('etaBarFill');
  const txt = document.getElementById('etaText');
  if(!box || !bar || !txt) return;

  box.style.display = 'block';
  const list = (orders || []).slice().sort((a,b) => Number(a.position || 9999) - Number(b.position || 9999));
  if(!list.length){
    bar.style.width = '0%';
    txt.textContent = '0s';
    window.__topEtaOrderKey = null;
    window.__topDenom = 0;
  ...
  const active = list.find(o => String(o.status || '').toLowerCase() === 'in progress') || null;
  const pending = list.find(o => String(o.status || '').toLowerCase() !== 'in progress') || null;
  const lastPlaced = lastOrderId ? list.find(o => String(o.orderId || o.id || '') === String(lastOrderId)) : null;
  const sameName = !lastPlaced && lastOrderName ? [...list].reverse().find(o => String(o.drinkName || o.drink || '') === String(lastOrderName)) : null;
  const chosen = lastPlaced || sameName || pending || active || first;

  const chosenEtaRaw = (chosen.etaSeconds == null ? 0 : chosen.etaSeconds);
  const chosenAheadRaw = (chosen.etaAheadSeconds == null ? 0 : chosen.etaAheadSeconds);
  const chosenStatus = String(chosen.status || '').toLowerCase();
  const remaining = __adjSec(chosenEtaRaw);
  const startsIn = chosenStatus === 'in progress' ? 0 : __adjSec(chosenAheadRaw);

  const orderKey = String(chosen.orderId || chosen.id || chosen.drinkName || 'top');
  const suggestedDenom = Math.max(1,
    Number(chosen.stepSeconds || 0),
    Number(chosenEtaRaw || 0),
    remaining,
    remaining + Math.max(0, startsIn)
  );

  if(window.__topEtaOrderKey !== orderKey){
    window.__topEtaOrderKey = orderKey;
    window.__topDenom = suggestedDenom;
  }else{
    window.__topDenom = Math.max(Number(window.__topDenom || 0), suggestedDenom);
  }

  const denom = Math.max(1, Number(window.__topDenom || suggestedDenom || 1));
  const pct = Math.max(0, Math.min(100, Math.round(((denom - remaining) / denom) * 100)));
  bar.style.width = pct + '%';

  const leadDrink = chosen.drinkName || chosen.drink || pending?.drinkName || active?.drinkName || first.drinkName || localStorage.getItem('lastOrderName') || '';
  const prefix = leadDrink ? `${leadDrink} • ` : '';
  const suffix = ` (starts in ${formatETA(Math.max(0, startsIn))})`;
  txt.textContent = `${prefix}${formatETA(remaining)}${suffix}`;
  if(!leadDrink && list.length){
    txt.textContent = `${formatETA(remaining)}${suffix}`;
  }
  try{
    if(leadDrink) localStorage.setItem('lastOrderName', leadDrink);
  }catch(_e){}
}
...
function __renderMyQueue(box){
  ...
  box.innerHTML = orders.map(o => {
    const norm = __normalizedQueueOrder(o);
    const drink = escapeHtml(norm.displayDrink || 'Drink');
    const qty   = o.quantity || 1;
    const status = escapeHtml(prettyStatus(o.status || 'Pending'));
    const pos = o.position || 1;

    const startsTxt = ` (starts in ${formatETA(norm.startsIn)})`;
    const rightTxt = `${formatETA(norm.eta)}${startsTxt}`;

    let bar = '';
    if(norm.status === 'in progress'){
      const done = Math.min(1, Math.max(0, (norm.total - norm.eta) / norm.total));
...
async function loadMyQueue(){
  ...
  try{
    ...
    const orders = __stabilizeOrders(data.orders || []);
    ...
    if(!orders.length){
      ...
      __myOrdersSnapshot = [];
      __mySnapshotTs = Date.now();
      try{ setMainEtaFromOrders([]); }catch(_e){}
      return;
    }
...
let __queueMonotonic = {};
let __queueLoaded = false;

function __stabilizeOrders(rawOrders){
  const now = Date.now();
  const seen = {};
  const out = (rawOrders || []).map((o, idx) => {
    const key = String((o && (o.orderId || o.id || o.drinkName)) || idx);
    seen[key] = true;
    let eta = Number((o && o.etaSeconds) ?? 0);
    let ahead = Number((o && o.etaAheadSeconds) ?? 0);
    if(!Number.isFinite(eta)) eta = 0;
    if(!Number.isFinite(ahead)) ahead = 0;
    const prev = __queueMonotonic[key];
    if(prev){
      const decay = Math.max(0, Math.floor((now - Number(prev.ts || now)) / 1000));
      const maxEta = Math.max(0, Number(prev.eta || 0) - decay + 1);
      const maxAhead = Math.max(0, Number(prev.ahead || 0) - decay + 1);
      if(eta > maxEta) eta = maxEta;
      if(ahead > maxAhead) ahead = maxAhead;
    }
    const stabilized = Object.assign({}, o || {}, { etaSeconds: Math.max(0, Math.round(eta)), etaAheadSeconds: Math.max(0, Math.round(ahead)) });
    __queueMonotonic[key] = { eta: stabilized.etaSeconds, ahead: stabilized.etaAheadSeconds, ts: now };
    return stabilized;
  });
  Object.keys(__queueMonotonic).forEach(k => { if(!seen[k]) delete __queueMonotonic[k]; });
  return out;
}
...
function __normalizedQueueOrder(o){
  const statusRaw = String((o && o.status) || '').toLowerCase();
  const etaRaw = Number((o && o.etaSeconds) ?? 0);
  const aheadRaw = Number((o && o.etaAheadSeconds) ?? 0);
  const status = statusRaw || 'pending';
  const eta = __adjSec(isFinite(etaRaw) ? etaRaw : 0);
  const startsIn = status === 'in progress' ? 0 : __adjSec(isFinite(aheadRaw) ? aheadRaw : 0);
  const total = Math.max(1, Number((o && o.stepSeconds) || etaRaw || eta || 1));
  const displayDrink = (o && (o.drinkName || o.drink)) || localStorage.getItem('lastOrderName') || '';
  return {
    raw: o || {},
    orderId: String((o && (o.orderId || o.id)) || displayDrink || 'top'),
    displayDrink,
    status,
    eta,
    startsIn,
...
function startMyQueueAutoRefresh(){
  ...
  loadMyQueue();
  myQueueTimer = setInterval(loadMyQueue, 3000);
  if(!__localTickTimer){
...
function renderEtaBar(){
  ...
  if(window.__queueLoaded){
    try{ setMainEtaFromOrders(Array.isArray(window.__myOrdersSnapshot) ? window.__myOrdersSnapshot : []); }catch(_e){}
    return;
  }
  ...
  const etaText = document.getElementById('etaText');
  const queuedLead = Array.isArray(window.__myOrdersSnapshot) && window.__myOrdersSnapshot.length
    ? ((window.__myOrdersSnapshot[0] && (window.__myOrdersSnapshot[0].drinkName || window.__myOrdersSnapshot[0].drink)) || '')
    : '';
  const lastItems = (() => { try { return JSON.parse(localStorage.getItem('lastOrderItems') || '[]'); } catch(_e) { return []; } })();
  const leadDrink = queuedLead || (Array.isArray(lastItems) && lastItems.length ? (lastItems[0].drinkName || '') : (localStorage.getItem('lastOrderName') || ''));
  const prefix = leadDrink ? `${leadDrink} • ` : '';
  if(etaText) etaText.innerText = `${prefix}${formatETA(Math.ceil(remaining))}${startsTxt}`;
}
...
// UI feedback
  ...
  try {
    ...
    if(data && data.ok){
      ...
      try{ localStorage.setItem('lastOrderItems', JSON.stringify(items)); localStorage.setItem('lastOrderName', (items[0] && items[0].drinkName) ? items[0].drinkName : ''); }catch(e){}
      cart = {};
      renderCart();
  ...
  pollTimer = setInterval(async ()=>{
    try{
      ...
      if(data && data.ok){
        ...
        pollTimer = null;
        clearEtaUi();
        const q = document.getElementById('myQueue');
        if(q) q.innerHTML = '<div class="small">No active orders.</div>';
      }
    }catch(e){ /* ignore */ }
  }, 3000);
}
  ...
  try{
    ...
    orders.forEach(o=>{
      ...
      html += `<tr>
        ...
        <td>${fmt(ahead)}</td>
        <td>${fmt(eta)}</td>
        <td class='small'>${o.drinkName || o.drinkId || ((o.orderId||'').slice(0,8) + '…')}</td>
      </tr>`;
    });
    html += "</table>";
    el.innerHTML = html;
