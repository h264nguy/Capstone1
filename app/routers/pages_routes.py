...
function setMainEtaFromOrders(orders){
  ...
  const chosen = lastPlaced || sameName || active || first;
  ...
  const startsIn = String(chosen.status || '').toLowerCase() === 'in progress' ? 0 : __adjSec(chosenAheadRaw);
  ...
  const leadDrink = chosen.drinkName || chosen.drink || first.drinkName || localStorage.getItem('lastOrderName') || '';
...
function renderEtaBar(){
  ...
  if(Array.isArray(window.__myOrdersSnapshot) && window.__myOrdersSnapshot.length){
    try{ setMainEtaFromOrders(window.__myOrdersSnapshot); }catch(_e){}
    return;
  }
