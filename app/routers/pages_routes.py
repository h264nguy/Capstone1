...
<body><div class='page'>
  ...
  <div class='card'>
    ...
    <div class='voice-panel'>
      <button id='voiceBtn' class='voice-btn' type='button'>Start Voice Ordering</button>
      <div id='voiceHint' class='small voice-hint'>
        Say: <strong>“Hey BMO, add one Voltage Fizz”</strong>, <strong>“add two Cola Spark”</strong>,
        <strong>“remove one Sprite”</strong>, <strong>“checkout”</strong>, or <strong>“clear cart”</strong>.
        <span id='voiceBadge' class='voice-badge'>Voice off</span>
      </div>
    </div>
...
function findDrinkFromVoice(phrase){
  ...
  if(!q) return null;
  const norm = s => normalizeVoiceText(String(s || '').replace(/-/g,' '));
  let best = null;
  let bestLen = -1;
  (drinks || []).forEach(d => {
    const dn = norm(d.name || d.id || '');
    if(!dn) return;
    if(q.includes(dn) || dn.includes(q)){
      if(dn.length > bestLen){
        best = d;
        bestLen = dn.length;
      }
    }
...
function handleVoiceCommand(rawText){
  ...
  if(/\b(check ?out|place order|order now)\b/.test(text)){
    bmoSay('Checking out');
    checkout();
    return;
  }

  if(/\b(clear cart|cancel order|empty cart)\b/.test(text)){
    cart = {};
    rerenderAfterVoice();
  ...
  if(/\b(add|order|make|get)\b/.test(text)){
  ...
  const fallbackDrink = findDrinkFromVoice(text);
  if(fallbackDrink){
    addDrinkByVoice(fallbackDrink, qty);
    return;
  }
