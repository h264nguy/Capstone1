    ...
    <div id='etaBox' class='etaBox' style='display:none;'>
      <div class='etaTop'>
        <div>
          ...
          <div id='etaDrinkName' class='small' style='margin-top:6px; opacity:.9'>No active order</div>
        </div>
...
function hasFallbackEtaState(){
  const lastOrderId = localStorage.getItem('lastOrderId');
  const etaRemaining = parseEtaSeconds(localStorage.getItem('etaRemaining'));
  return !!(lastOrderId && etaRemaining !== null && etaRemaining > 0);
}
...
function setMainEtaFromOrders(orders){
  ...
  const nameEl = document.getElementById('etaDrinkName');
  if(!box || !bar || !txt) return;

  box.style.display = 'block';
  const list = (orders || []);
  if(!list.length){
    if(hasFallbackEtaState()){
      if(nameEl){
        const fallbackName = localStorage.getItem('lastOrderName') || 'Drink';
        nameEl.textContent = `Tracking: ${fallbackName}`;
      }
      return;
    }
    bar.style.width = '0%';
  ...
  const sorted = [...list].sort((a,b) => (parseInt(a.position || 999999) - parseInt(b.position || 999999)));
  const lead = sorted.find(o => o.status === 'In Progress') || sorted[0] || null;
  if(nameEl) nameEl.textContent = lead ? `Tracking: ${lead.drinkName || lead.drink || 'Drink'}` : 'No active order';
  ...
  try {
    ...
    if(data && data.ok){
      ...
      try{
        ...
        const firstName = (firstItem && (firstItem.name || firstItem.drinkName || firstItem.id)) ? (firstItem.name || firstItem.drinkName || firstItem.id) : '';
        if(firstName) localStorage.setItem('lastOrderName', String(firstName));
      }catch(e){}
      cart = {};
      renderCart();
