...
function setMainEtaFromOrders(orders){
  ...
  if(!list.length){
    ...
    bar.style.width = '0%';
    txt.textContent = '0s';
    window.__topEtaState = null;
    return;
  }

  const lastOrderId = localStorage.getItem('lastOrderId') || '';
  const lastOrderName = localStorage.getItem('lastOrderName') || '';
  let chosen = lastOrderId ? list.find(o => String(o.orderId || o.id || '') === String(lastOrderId)) : null;
  if(!chosen && lastOrderName){
    chosen = [...list].reverse().find(o => String(o.drinkName || o.drink || '') === String(lastOrderName)) || null;
  }
  if(!chosen) chosen = list[0] || {};

  const norm = __normalizedQueueOrder(chosen);
  const orderKey = norm.orderId || String(chosen.drinkName || chosen.drink || 'top');
  const suggestedTotal = Math.max(1, Number(chosen.estSeconds || chosen.stepSeconds || norm.total || 1), norm.eta + Math.max(0, norm.startsIn));

  if(!window.__topEtaState || window.__topEtaState.key !== orderKey){
    window.__topEtaState = {
      key: orderKey,
      total: suggestedTotal,
      drink: norm.displayDrink || String(chosen.drinkName || chosen.drink || ''),
    };
  } else {
    window.__topEtaState.total = Math.max(1, Number(window.__topEtaState.total || 1), suggestedTotal);
    if(norm.displayDrink) window.__topEtaState.drink = norm.displayDrink;
  }

  const total = Math.max(1, Number(window.__topEtaState.total || suggestedTotal || 1));
  const remaining = Math.max(0, Number(norm.eta || 0));
  const startsIn = Math.max(0, Number(norm.startsIn || 0));
  const pct = Math.max(0, Math.min(100, Math.round(((total - remaining) / total) * 100)));

  box.style.display = 'block';
  bar.style.width = `${pct}%`;

  const leadDrink = window.__topEtaState.drink || norm.displayDrink || '';
  const prefix = leadDrink ? `${leadDrink} • ` : '';
  txt.textContent = `${prefix}${formatETA(remaining)} (starts in ${formatETA(startsIn)})`;
  try{ if(leadDrink) localStorage.setItem('lastOrderName', leadDrink); }catch(_e){}
}
...
function __stabilizeOrders(rawOrders){
  ...
  const out = (rawOrders || []).map((o, idx) => {
    ...
    if(prev){
      ...
      const maxEta = Math.max(0, Number(prev.eta || 0) - decay);
      const maxAhead = Math.max(0, Number(prev.ahead || 0) - decay);
      if(prev && eta > maxEta) eta = maxEta;
      if(prev && ahead > maxAhead) ahead = maxAhead;
    }
...
// UI feedback
  ...
  try {
    ...
    if(data && data.ok){
      ...
      if(etaBox){ setTimeout(() => etaBox.scrollIntoView({behavior:'smooth', block:'start'}), 220); }}else{
