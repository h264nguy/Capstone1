...
function setMainEtaFromOrders(orders){
  ...
  const first = list[0] || {};
  const active = list.find(o => String(o.status || '').toLowerCase() === 'in progress') || first;
...
function startMyQueueAutoRefresh(){
  ...
  loadMyQueue();
  myQueueTimer = setInterval(loadMyQueue, 1000);
  if(!__localTickTimer){
...
function scrollToCheckoutStatus(){
  const targets = [document.getElementById('etaBox'), document.getElementById('myQueueBox'), document.getElementById('status')].filter(Boolean);
  const target = targets.find(el => el && (el.offsetParent !== null || el.id === 'status')) || targets[0];
  if(!target) return;
  try{ target.scrollIntoView({behavior:'smooth', block:'center'}); }catch(_e){}
  setTimeout(() => { try{ target.scrollIntoView({behavior:'smooth', block:'center'}); }catch(_e){} }, 500);
  setTimeout(() => { try{ target.scrollIntoView({behavior:'smooth', block:'center'}); }catch(_e){} }, 1200);
}
...
try{ scrollToCheckoutStatus(); }catch(e){}
    }else{
  ...
  pollTimer = setInterval(async ()=>{
    try{
      ...
      if(data && data.ok){
        ...
        try{ scrollToCheckoutStatus(); }catch(_e){}
      } else {
      ...
      if(rdata && rdata.ok){
        ...
        if(banner){
          ...
          if(based && hasRecs){
            ...
            banner.innerHTML = `Because you ordered: <strong>${escapeHtml(based)}</strong>`;
          }else{
