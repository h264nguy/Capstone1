...
function setMainEtaFromOrders(orders){
  ...
  const lead = list.slice().sort((a,b) => Number(a.position || 9999) - Number(b.position || 9999))[0] || {};
  const leadDrink = lead.drinkName || lead.drink || localStorage.getItem('lastOrderName') || '';
...
function renderEtaBar(){
  ...
  const leadDrink = Array.isArray(lastItems) && lastItems.length ? (lastItems[0].drinkName || '') : (localStorage.getItem('lastOrderName') || '');
  const prefix = leadDrink ? `${leadDrink} • ` : '';
