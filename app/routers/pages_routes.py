...
function __isActiveQueueOrder(o){
  ...
  const eta = (o && o.etaSeconds==null) ? 0 : __adjSec(o.etaSeconds || 0);
  const ahead = (o && o.etaAheadSeconds==null) ? 0 : __adjSec(o.etaAheadSeconds || 0);
  ...
  // Queued / pending drinks are still active if they still have remaining wait
  // or completion time.
  if(eta > 0 || ahead > 0) return true;

  // Only keep pending / queued rows alive when they still have actual time left.
  return false;
}
...
function setMainEtaFromOrders(orders){
  ...
  const list = (orders || []).filter(isOrderStillActive).slice().sort((a,b)=>{
  ...
  // Match the live-display machine: prefer the currently-making drink,
  // otherwise show the next drink for this user by queue position.
  const inProgress = list.find(o => String(o.status||'').toLowerCase().replace(/\s+/g,'') === 'inprogress');
  const target = inProgress || list[0];
  const targetId = String(target.orderId || target.id || '');
  const startsIn = inProgress ? 0 : Math.max(0, __adjSec(target.etaAheadSeconds || 0));
  const remainRaw = inProgress
    ? (target.etaThisSeconds ?? target.etaSeconds ?? 0)
    : (target.etaSeconds ?? target.etaThisSeconds ?? 0);
  const remain = Math.max(0, __adjSec(remainRaw || 0));
  ...
  // Stable denominator per selected order so the bar does not jump.
  if(!window.__topDenom || window.__topDenomTs !== __mySnapshotTs || window.__topEtaOrderId !== targetId){
    window.__topDenom = Math.max(1, remain);
...
async function loadMyQueue(){
  ...
  try{
    ...
    const orders = (data.orders || []).filter(__isActiveQueueOrder);
