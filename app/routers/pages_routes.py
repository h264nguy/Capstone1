...
function setMainEtaFromOrders(orders){
  ...
  const list = (orders || []).filter(__isActiveQueueOrder);
  if(!list.length){
    bar.style.width = '0%';
    txt.textContent = '';
  ...
  const startsIn = list.some(o => String(o.status||'').toLowerCase().replace(/\s+/g,'') === 'inprogress') ? 0 : Math.max(0, Math.min(...aheads));

  if(totalRemain <= 0 && startsIn <= 0){
    bar.style.width = '0%';
    txt.textContent = '';
    box.style.display = 'none';
    return;
