...
def _pretty_label(raw: str) -> str:
    return " ".join(
        part[:1].upper() + part[1:]
        for part in str(raw or "").replace("-", " ").replace("_", " ").split()
    )
...
def _enrich_status(raw: dict) -> dict:
    ...
    if ingredients:
        existing = s.get("ingredients") or []
        if not existing:
            s["ingredients"] = ingredients
        else:
            s["ingredients"] = [_pretty_ingredient_name(x) for x in existing if str(x or "").strip()]

    if meta.get("name"):
        current_drink = str(s.get("drink") or "").strip()
        if not current_drink or current_drink in ingredients:
            s["drink"] = meta["name"]
    ...
    machine = str(s.get("machine") or "").lower()

    current_ing = str(s.get("currentIngredient") or "").strip()
    if current_ing:
        current_ing = _pretty_ingredient_name(current_ing)

    # Only auto-fill a current ingredient once the machine is actually making the drink.
    if not current_ing and ingredients and machine not in {"idle", "waiting", "queued", "done", "cooldown"} and stage not in {"idle", "waiting", "queued", "done", "cooldown"}:
        current_ing = ingredients[0]
    s["currentIngredient"] = current_ing

    idx = int(s.get("currentIngredientIndex") or 0)
    if idx <= 0 and current_ing and s.get("totalIngredients") and machine not in {"idle", "waiting", "queued", "done", "cooldown"}:
        idx = 1
    ...
    if oid:
        ...
        if not s.get("headline") and stage == "queued":
