...
def _enrich_status(raw: dict) -> dict:
    ...
    if not s.get("ingredients") and ingredients:
    ...
    if not current_ing and ingredients and stage in {"pouring", "finishing", "making"}:
        idx_raw = s.get("currentIngredientIndex")
        try:
            idx = int(idx_raw or 0)
        except Exception:
            idx = 0
        # support both 0-based and 1-based senders
        if idx <= 0:
            pick = 0
        else:
            pick = min(len(ingredients) - 1, idx - 1)
