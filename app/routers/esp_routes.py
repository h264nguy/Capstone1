    ...
    if oid:
        ...
        if qinfo:
            ...
            if qstatus == "In Progress":
                ...
                eta_this = int(qinfo.get("etaThisSeconds") or qinfo.get("etaSeconds") or status.get("remainingSeconds") or 0)
                status["remainingSeconds"] = eta_this
                est = int(qinfo.get("estSeconds") or 0)
                if est > 0:
                    inferred_progress = max(1, min(99, int(round((1 - (eta_this / max(1, est))) * 100))))
                    status["progress"] = max(int(status.get("progress") or 0), inferred_progress)
                    total_ing = int(status.get("totalIngredients") or len(status.get("ingredients") or []) or 0)
                    if total_ing > 0:
                        inferred_step = max(1, min(total_ing, (inferred_progress * total_ing + 99) // 100))
                        current_idx = int(status.get("currentIngredientIndex") or 0)
                        status["currentIngredientIndex"] = max(current_idx, inferred_step)
            elif machine == "queued":
