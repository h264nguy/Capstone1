...
## New iPad Status Display

This version adds a live status screen for your iPad.

### New routes
- `/status` → full-screen iPad dashboard
- `/api/status` → current machine status JSON
- `/api/esp/status?key=ESP_POLL_KEY` → ESP posts live updates here

### What the iPad shows
- Current drink name
- Current ingredient being poured
- Step number
- Remaining time
- Live progress bar

### Setup
1. Deploy this updated backend.
2. Update your ESP sketch with your Wi-Fi, Render URL, and `ESP_POLL_KEY`.
3. Open `https://YOUR-APP.onrender.com/status` on your iPad.
4. Keep the iPad beside the machine during demo day.


Merged build: BMO-inspired live machine display + stable cart ETA/menu timing logic.
